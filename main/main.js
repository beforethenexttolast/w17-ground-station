...
// the quit policy.
const { wifi, hotspotLifecycle, adapterMonitor, sim } = createNetworkServices({ env: process.env, log });
// One broadcaster for every one-way main -> renderer push; the channel name is
// always a PUSH_CHANNELS constant supplied by the wiring seam, never a literal.
const broadcast = (channel, snap) => {
  for (const win of BrowserWindow.getAllWindows()) {
    if (!win.isDestroyed()) win.webContents.send(channel, snap);
  }
};
wireHotspotPush({ lifecycle: hotspotLifecycle, broadcast });
// Live adapter monitor (2B) push + the adapter/hotspot coordinator (2C/2D): the
// coordinator is the ONLY caller of hotspotLifecycle.markInterrupted and the
// re-verify-on-adapter-change trigger. Both are constructed here, at the same
// composition root as the hotspot push; the monitor is STARTED in whenReady.
wireAdapterPush({ monitor: adapterMonitor, broadcast });
createAdapterCoordinator({ adapterMonitor, hotspotLifecycle, log });
...
function createWindow() {
  ...
  // Full screen for the packaged pit-wall build (2A); a dev run stays windowed
  // unless W17_FULLSCREEN overrides. NOT kiosk — the frame + F11 restore stay.
  const fullscreen = resolveFullscreen({ isPackaged: app.isPackaged, env: process.env });
  ...
  // Operator full-screen restore/exit (2A): Electron has no built-in F11 on
  // Windows, so decide the action from the raw key event (pure helper) and
  // toggle. The renderer never sees this — it is a chrome affordance, not an
  // app input. Full screen is a window property, so the single-page step
  // switches and any renderer reload keep it; only this key changes it.
  win.webContents.on('before-input-event', (event, input) => {
    const action = fullscreenKeyAction(input, { isFullScreen: win.isFullScreen() });
    if (!action) return;
    win.setFullScreen(action === 'toggle' ? !win.isFullScreen() : false);
...
app.whenReady().then(async () => {
  ...
  // Start the live adapter monitor only where WLAN scanning exists at all
  // (Windows, or the dev sim) — on a plain macOS/Linux dev host there are no
