...
| `10-install-gs.ps1` | `-InstallerPath` | silent-installs the NSIS build; FAILS LOUDLY if `mediamtx.exe` or the packaged `proto/` is missing. **The meaning of that FAIL has flipped**: `boundaries-1` and `boundaries-6` are FIXED on `main` (`.github/workflows/ci.yml:79` fetches mediamtx before packaging, `:85` runs `scripts/assert-packaged.js`, and `electron-builder.yml:14` lists `proto/**`), so a miss here is now a **regression**, or something the NSIS installer drops that CI's `dist\win-unpacked` assertion does not catch — not a known defect reproducing. |
...
| `50-race-day.ps1` | `-InstallDir` (+ a completed `20` stage) | drives race day's REAL mapper step end to end, to reproduce CONFIRMED blockers `MAP-1` (config double-wrap panic) and `MAP-2` (RF link structurally never started), plus `MAP-8` (gRPC :10000 / webapp :3000 reachable while the mapper runs). `boundaries-4`/`boundaries-5` are recorded here as **closed** — both spawn sites now share one case-insensitive `W17_*` scrub (`shared/childEnv.js:32-35`, used at `main/elrsLauncher.js:55` and `main/mapperRunner.js:68-70`). `MAP-2`/`SYN-2` reproduce on **every** run until the fix wave lands, so they are recorded in `data.expectedFindingsReproduced` and do **not** set the exit code: a FAIL here means something the suite did *not* already know about. |
